define(function () {
    ...
    tg.loadTemplateForm = function (content) {
        ...
        for (i = 0; i < ltags; i++) {
            (d.getElementById(tags[i]) === null) && this.createField(tags[i]);
        }
    };
    tg.process = function (evt) {
        var inputs, output, i, re, d, linputs, output_html, ip, content;
        evt.stopPropagation();
        evt.preventDefault();
        d = document;
    ...
    tg.createField = function (id) {
        var d, category, fieldset, legend, list, li, fieldlist;
        d = document;
        category = id.match(/\w+(?=:)/);
